...
async def setup_bot_commands():
    ...
    try:
        ...
        LOGGER("SparshXshizuka").info("Bot commands set successfully!")
        
    except Exception as e:
        LOGGER("SparshXshizuka").error(f"Failed to set bot commands: {str(e)}")
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("SparshXshizuka.plugins" + all_module)

    LOGGER("sparshXshizuka.plugins").info("Successfully Imported Modules...")

    await userbot.start()
    await Nand.start()

    try:
        await sparsh.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("SparshXshizuka").error(
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("SparshXshizuka").info("Stopping Shizuka Music Bot...🥺")
